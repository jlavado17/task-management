        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="/projects" class="flex flex-col space-y-8" method="POST">
                        @csrf
                        <div>
                            <h3 class="text-1xl font-semibold">New Project</h3>
                            <hr>
                        </div>
                        <div class="form-item">
                            <label class="text-md">Name</label>
                            <input type="text" name="name" class="w-full appearance-none text-black text-opacity-70 rounded shadow py-1 px-2  mr-2 focus:outline-none focus:shadow-outline focus:border-blue-200">
                        </div>
                        <div class="form-item">
                            ...
                            <select name="priority" class="w-full appearance-none text-black text-opacity-70 rounded shadow py-1 px-2  mr-2 focus:outline-none focus:shadow-outline focus:border-blue-200">
                                @for ($i = 1; $i <= $totalProjects; $i++)
                                    <option value="{{ $i }}" {{ $i == $totalProjects ? 'selected' : '' }}>{{ $i }}</option>
                                @endfor
                            </select>
                        </div>
                        <div class="p-6 ">
